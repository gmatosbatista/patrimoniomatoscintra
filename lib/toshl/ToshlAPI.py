...
from lib.toshl.ToshlConfig import *
...
class ToshlApi:
    ...
    def fetch_dxfeed_token(self) -> bool:
        ...
        self.streamer_uri = response["data"]["dxlink-url"]
    ...
    def fetch_transactions(self, account: str) -> list:
        url = f"{self.tt_uri}/accounts/{account}/transactions"
        headers = {
        ...
        try:
            ...
            total_pages = response_json['pagination']['total-pages']
            ...
            for page_offset in range(total_pages):
                ...
                response.raise_for_status()
                response_json = response.json()
                transactions_data.extend(response_json['data']['items'])
